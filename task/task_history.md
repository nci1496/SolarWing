...
- 2026-06-10：Phase 7 参数保存 / 读取首轮落地。保留原有对话框各自的本机 `QSettings` 偏好记忆，同时新增面向课程验收的独立 `ini` 场景配置闭环：在主窗口“设置”菜单加入“保存配置 / 打开配置”，将基本设置与参数设置统一写入单个 `INI` 文件，并支持从该文件恢复积分器、曲线显示参数和仿真参数，随后自动同步回对话框、刷新曲线与动画，为后续直接产出 `StepTest.ini` / `SinTest.ini` 打下基础。涉及文件：`mainwindow.h`、`mainwindow.cpp`、`dialogs/basicsettingsdialog.h`、`dialogs/basicsettingsdialog.cpp`、`dialogs/paramsettingsdialog.h`、`dialogs/paramsettingsdialog.cpp`、`task/task_history.md`、`task/current_task.md`。
- 2026-06-10：参数交互一致性修正。针对新一轮联调中暴露的两个问题完成修复：1）PID 参数区此前滑条按 `/100` 映射，而默认参数与仿真内部期望量级为 `Kp=100, Ki=25, Kd=45`，导致滑条与数值框严重不一致；现已统一改为滑条 `0~1000` 对应参数 `0.0~100.0` 的 `/10` 映射，并同步调整界面默认值与回写逻辑。2）“基本设置”此前只有积分器选择真正参与仿真，曲线背景色、文字色和字体并未实际应用到图表；现已为 `PlotWidget` 增加显示样式下发接口，主窗口会将基本设置中的背景颜色、文字颜色和字体同步应用到三张曲线图，使设置项具备真实可见效果。涉及文件：`mainwindow.cpp`、`dialogs/paramsettingsdialog.cpp`、`dialogs/paramsettingsdialog.ui`、`widgets/plotwidget.h`、`widgets/plotwidget.cpp`、`task/task_history.md`。
